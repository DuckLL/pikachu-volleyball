...
function letAIDecideUserInput(player, ball, theOtherPlayer, userInput) {
  ...
  if (player.tactics === 0) {
    ...
    for (let frame = 0; frame < ball.path.length; frame++) {
      ...
      if (cantouch(theOtherPlayer, ball.path[frame], frame)) {
        maychange = true;
    ...
    if (player.yVelocity === 16 && player.y === 244) {
    ...
    if (
      // samesideloss(theOtherPlayer, ball.expectedLandingPointX) ||
      ...
      // attack
      if (!thunder_defense && player.secondattack < 0) {
        let shortPath = 1000;
        // jumping ball from x;
        if (
          player.goodtime !== 0 &&
            ...
            for (
              ...
              if (
                ...
                Math.abs(predictball.x - player.x) <=
                  6 * predictframe + PLAYER_HALF_LENGTH - (direct === 3 ? 6 : 0)
              ) {
                shortPath = -1;
                player.goodtime = 0;
                const shift =
            ...
            for (let direct = 0; capability.freestyle && direct < 2; direct++) {
              ...
              for (
                ...
                if (
                  ...
                  player.secondattack = predictframe + 1;
                  console.log(
                    (player.isPlayer2 ? '2' : '1') + ':0sec freestyle'
                  );
                  break;
                }
              }
                ...
                for (
                  ...
                  for (
                    ...
                    if (
                      ...
                      if (
                        direct === 2 &&
                        Math.abs(predictball.x - GROUND_HALF_WIDTH) > 30 &&
                      ...
                      player.secondjump = false;
                      console.log(
                        (player.isPlayer2 ? '2' : '1') + ':normal freestyle'
                      );
                      break;
                    }
                  }
    ...
    if (player.goodtime === 0 || player.secondattack === 0) {
      userInput.powerHit = 1;
      // block
      // if (Math.abs(ball.x - GROUND_HALF_WIDTH) < 72 && ball.isPowerHit) {
      //   userInput.powerHit = 0;
      //   player.goodtime = -1;
      //   player.secondattack = -1;
      // }
      if (!player.freestyle) {
          ...
          for (let direct = 0; capability.freestyle && direct < 6; direct++) {
            ...
            for (
              ...
              if (
                ...
                if (
                  shortPath === -1 &&
                  (direct === 2 || direct === 3 || player.direction === 4)
                ) {
                  break;
                }
                shortPath = -1;
                ...
                player.secondattack = predictframe + 1;
                console.log(
                  (player.isPlayer2 ? '2' : '1') + ':second freestyle'
                );
                break;
              }
            }
              ...
              if (
                ...
                for (let direct = 0; direct < 6; direct++) {
                  ...
                  if (
                    ...
                    predict.length <= short_path &&
                    (direct > 3 || !canblock(theOtherPlayer, predict))
                  ) {
                    short_path = predict.length;
                    player.direction = direct;
                    canbypass = true;
        ...
        if (
          ...
          userInput.powerHit = 0;
          player.secondattack = -1;
          console.log('cancel 0 freestyle');
        }
        player.freestyle = false;
      }
